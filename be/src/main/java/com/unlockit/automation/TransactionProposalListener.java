...
@ApplicationScoped
public class TransactionProposalListener {

    private static final Logger LOG = Logger.getLogger(TransactionProposalListener.class);
    ...
    @Inject
    TransactionProposalProcessor processor;

    private ManagedChannel channel;
    private final Random random = new Random();

    void onStart(@Observes StartupEvent ev) {
        LOG.infof("Starting TransactionProposalListener - connecting to Canton gRPC at %s:%d", grpcHost, grpcPort);
    ...
    void onStop(@Observes ShutdownEvent ev) {
        LOG.info("Shutting down TransactionProposalListener");
    ...
    private void startEventStream() {
        try {
            ...
            var contractFilter = TransactionSubmissionProposal.contractFilter();
            ...
            LOG.info("Starting to stream TransactionSubmissionProposal events...");
        ...
        response.getTransaction().ifPresent(transaction -> {
            ...
            for (Event event : transaction.getEvents()) {
                if (event instanceof CreatedEvent) {
                    ...
                    try {
                        TransactionSubmissionProposal.Contract contract =
                                TransactionSubmissionProposal.Contract.fromCreatedEvent(createdEvent);

                        LOG.infof("New TransactionSubmissionProposal created! Contract ID: %s, Transaction ID: %s",
                                contract.id.contractId,
                                contract.data.transactionId);

                        processProposal(contract);

                    } catch (Exception e) {
                        LOG.debugf("Event is not a TransactionSubmissionProposal: %s", e.getMessage());
                    }
                }
            }
        });
    ...
    private void processProposal(TransactionSubmissionProposal.Contract contract) {
        boolean accept = random.nextInt(100) < 80;

        if (accept) {
            LOG.infof("Accepting TransactionSubmissionProposal %s (80%% chance)", contract.data.transactionId);
            processor.acceptProposal(contract);
        } else {
            String[] reasons = {
                    "Missing required documentation",
                    "Property address verification failed",
                    "Sale price appears unrealistic",
                    "Duplicate transaction detected",
                    "Incomplete property information"
            };
            String reason = reasons[random.nextInt(reasons.length)];
            LOG.infof("Rejecting TransactionSubmissionProposal %s - Reason: %s (20%% chance)",
                    contract.data.transactionId, reason);
            processor.rejectProposal(contract, reason);
        }
    }
}
